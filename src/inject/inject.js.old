...
function importFile(file) {
    ...
    if (file.name.match(textType)) {
        ...
        $("#process").hide();
        showErrorMessage("ERROR1: only csv with header 'name,acquired,sold,proceeds,cost' is supported!");
    }
}
...
function processData(allText) {
    ...
    if (headers[0] != 'Name' || headers[1] != 'DATE ACQUIRED' || headers[2] != 'SALE DATE' || headers[3] != 'SALES PRICE' || headers['4'] != 'COST BASIS') {
        showErrorMessage("ERROR2: only csv with header 'name,acquired,sold,proceeds,cost' is supported!");
        return false;
    }
    var lines = [];
    for (var i=1; i<allTextLines.length; i++) {
        ...
        if (data.length == headers.length) {
            ...
            for (var j=0; j<headers.length; j++) {
                if (headers[j] == 'DATE ACQUIRED') {
                    var acquiredData = data[j].split(' ');
                    if (acquiredData && acquiredData[0].length > 1) { 
                        let dateString =  acquiredData[0];
                        if(data[j]){
                            // null check
                            data[j] = acquiredData[acquiredData[0].length - 1];
                            let year = dateString.substr(0, 4);
                            let month = dateString.substr(4, 2) - 1; // Month value starts from 0
                            ...
                            console.log(formattedDate); // Output: "March 12, 2022"
                            tarr.push(formattedDate);
                        }
                    }
                }
...
function validateDate(dateString) {
    return dateString[0].match(/^(January|February|March|April|May|June|July|August|September|October|November|December)\s\d{1,2},\s\d{4}$/i) ? true : false;
}
...
function validateInputFile(lines) {
    ...
    for (var i = 0; i < lines.length; i++) {
        ...
        if (!lines[i] || !validateDate(lines[i])) {
